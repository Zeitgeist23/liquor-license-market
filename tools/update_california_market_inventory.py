...
def robots_allows(url):
    ...
    try:
        ...
        rp = robotparser.RobotFileParser(); rp.set_url(robots_url); rp.parse(response.text.splitlines())
        return rp.can_fetch(UA, url) and rp.can_fetch("*", url)
    except Exception:
        return True

def get_soup(url):
    response = requests.get(url, timeout=45, headers={"User-Agent": UA}); response.raise_for_status()
...
def scrape_alcoholpermit(source):
    ...
    for link in soup.find_all("a", href=True):
        ...
        text = clean(card.get_text(" ", strip=True)); match = pattern.search(text)
        if not match: continue
        county = clean(match.group(1)); type_code = match.group(2)
        if not county or len(county) > 40: continue
        price = money(text); private = "private listing" in text.lower() or "unlock the price" in text.lower()
        image = card.find("img", alt=True); image_alt = clean(image.get("alt")) if image else ""
        id_match = re.search(r"\bID\s*(\d+)\b", image_alt, re.I)
        title_el = card.find(["h2", "h3", "h4"]); title = clean(title_el.get_text(" ", strip=True)) if title_el else f"California Type {type_code} Liquor License – {county} County"
        records.append({"source":source["name"],"sourceKind":"third-party","county":county,"typeCode":type_code,"typeName":f"Type {type_code}","price":None if private else price,"privatePrice":private,"listedDate":None,"sourceId":id_match.group(1) if id_match else None,"title":title,"sourceUrl":href}); seen.add(href)
    return records, None
...
def scrape_marketplace(source):
    ...
    for link in soup.find_all("a", href=True):
        if clean(link.get_text(" ", strip=True)).lower() != "view listing": continue
        href = urljoin(url, link["href"])
        if href in seen: continue
        card = best_container(link, pattern)
        if not card: continue
        ...
        records.append({"source":source["name"],"sourceKind":"third-party","county":county,"typeCode":type_code,"typeName":f"Type {type_code}","price":price,"privatePrice":False,"listedDate":date_match.group(1) if date_match else None,"sourceId":None,"title":clean(match.group(0)),"sourceUrl":href}); seen.add(href)
    return records, None

def main():
    combined = []; source_status = []
    ...
    payload = {"updatedAt":datetime.now(timezone.utc).replace(microsecond=0).isoformat().replace("+00:00","Z"),"sources":source_status,"rawRecordCount":stats["rawRecordCount"],"uniqueCount":stats["uniqueCount"],"duplicatesMerged":stats["duplicatesMerged"],"mergedCards":stats["mergedCards"],"licenses":unique}
